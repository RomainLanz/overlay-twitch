<template>
	<div class="absolute top-0 left-0 right-0 mt-56 ml-32 text-2xl">
		<TransitionGroup
			class="space-y-4"
			enter-active-class="transition duration-300 ease-out"
			enter-from-class="transform opacity-0 -translate-x-28"
			enter-to-class="transform translate-x-0 opacity-100"
			leave-active-class="transition duration-200 ease-in"
			leave-from-class="transform translate-x-0 opacity-100"
			leave-to-class="transform opacity-0 -translate-x-28"
			move-class="transition duration-200 transform"
			tag="div"
		>
			<div class="flex justify-start" v-for="alert in alerts" :key="alert.uuid">
